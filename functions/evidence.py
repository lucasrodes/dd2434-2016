...
mean = 0
std = sqrt(1000)
Theta = prior_parameters(mean, std, S) # 3 x S
...
def evidence_model_theta(dataset, theta, model):
	...
	# Logistic regression, only considers x_1
		...
		return np.prod([1./(1+np.exp(-dataset[n]*theta[0]*x[n,0])) 
			for n in range(len(dataset))])
	# Logistic regression, considers both x_1 and x_2
	elif model == 2:
		return np.prod([1./(1+np.exp(-dataset[n]*(theta[0]*x[n,0]+theta[1]*x[n,1]))) 
			for n in range(len(dataset))])
	# Considers x_1, x_2 and a bias term
	elif model == 3:
		return np.prod([1./(1+np.exp(-dataset[n]*(theta[0]*x[n,0]+theta[1]*x[n,1] + theta[2]))) 
			for n in range(len(dataset))])
